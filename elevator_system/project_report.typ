#set document(title: "Elevator System - Group 10")
...
#figure(
  image("Module_interfaces.png"),
  caption: [Module interaction diagram.],
) <fig-modules>

#figure(
  image("order_lifecycle.png"),
...
The master executes order assignment, synchronizes the uncompleted-order list with peers, and instructs selected elevators to serve each order. The flow is shown in @fig-order-lifecycle:

+ Elevator informs master of button press.
+ Master informs all peers of the order.
+ All peers receive order.
+ Master informs peers order is to be served, with `QueueOrders`.
+ The master sends `AssignOrders` to the selected elevator.
+ Elevator completes order and sends `CompleteOrder`.
+ Master informs peers to `ClearOrders`.

When a new order arrives, `assign_new_order` designates busy and idle elevators, then applies two strategies in sequence:

+ *Interruption* (`pause_order`): If a busy elevator is currently traveling past the new hall call "on the way" (determined by `order_on_the_way`, which checks floor ordering and call direction), its current order is stashed at the front of the queue and the new order is assigned instead. This avoids the elevator ignoring a stop it could have served.

+ *Closest idle* (`find_closest_elev`): Among idle elevators, pick the one minimizing `abs_diff(floor, order.floor)`.
...
Building the module from scratch gave a big learning benefit, but using the provided cost function would be simpler and likely reduce complexity. Still, a custom system gave increased control of order management behaviour.
...
*Layer 1: Per-datagram reliability* (`transport.rs`): Each `Msg` is wrapped in a `Frame::Data { seq, msg }` and serialized with bincode. The sender retries every 5 ms until it receives a `Frame::Ack` matching both the sequence number _and_ the full message content, or gives up after a set amount of attempts. The receiver sends three copies of each ACK to reduce ACK-loss probability. Each send spawns its own Tokio task with its own UDP socket, so multiple reliable sends proceed concurrently.
...
- Pure functions from `assignment.rs` could be moved to a separate file to make the control flow clearer.
