...
async fn main() -> io::Result<()> {
    ...
    let elevator_runner_task = tokio::spawn(async move {
        elevator::elevator_runner(id, call_request_tx, call_assign_rx, update_floor_tx, call_complete_tx, call_light_assign_rx, master_notify_tx, elevs_alive_rx).await });
